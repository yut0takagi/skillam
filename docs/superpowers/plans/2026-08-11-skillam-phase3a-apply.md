...
各タスクの本文は当初の計画のまま残してある。実装とコードレビューの過程で見つかった問題と、その結果入った変更を以下に記録する。最終的な仕様は実装（およびそのテスト）が正。

### スキーマ（Task 1）

- `apply_history.role_id` を `NOT NULL ... ON DELETE CASCADE` から nullable + `ON DELETE SET NULL` に変更し、`projects.last_applied_role_id` にも `ON DELETE SET NULL` を追加した。当初のSQLでは `foreign_keys = ON` のもとで**適用済みロールを削除できず**、また削除できた場合は `managed_json`（skillamが何を書いたかの記録）まで消えて次回適用の掃除が壊れる

### 「黙って壊さない」原則の徹底

適用対象が解釈できない、あるいは skillam のものでない場合は、推測して書き換えず明示的に失敗する。以下の経路すべてで例外を投げる。

- `UnsupportedSettingsError`（`plan-settings.ts`）: 既存の `settings.json` / `.mcp.json` がJSONとして読めない、オブジェクトでない、`permissions` が非オブジェクト。当初の実装はいずれも `{}` として扱っており、**ユーザーのファイル内容を丸ごと消して上書き**していた
- `MaterializeConflictError`（`plan-materialize.ts`）: 適用先に skillam が作成していない実ファイル/ディレクトリがある、配置先がプロジェクト外を指す、複数のロール項目が同一パスに衝突する、リンク先が存在しない

とくに1点目の実ディレクトリ削除は、エグゼキュータの `rmSync(recursive)` と組み合わさって**ユーザーが手で作ったディレクトリを中身ごと消す**経路だった。`CurrentEntry` に `{ kind: 'other' }` を追加し、ディレクトリを「空のファイル」と偽らないようにして塞いだ。

### 適用の書き込み順と原子性（Task 11）

- シークレット解決 → 操作ループ → 設定ファイル書き込み、の順に変更。途中失敗時に設定ファイルが前回適用の状態のまま残り、不整合の範囲が小さくなる
- 書き込みは一時ファイル + `rename` で原子的に行う（`settings.json` の切り詰め防止）
- 書き込むバイト列がディスク上の内容と一致する場合は書き込みを省略する。判定には**復号後の内容**を使う（`secret_ref:` は不変なので、プレビュー文字列で比較するとシークレット更新が反映されない）
- env の非シークレット値は型を保ったまま素通しする（`String()` による変換はプレビューと実書き込みを食い違わせる）

### 履歴の欠落防止（Task 9・12）

- `diff` の直列化は失敗しない（循環参照を握りつぶす）。ファイル書き込み後に履歴行が残らないと、次回適用が自分の書いたものを識別できなくなるため
- 成功パス・失敗パスとも `history.record` を try/catch で保護し、記録できなかった場合は「ディスクと履歴が乖離した」ことを明示するエラーを返す
- `buildApplyPlan` は「最後の成功以降のすべての履歴行」の `managed` の和集合を使う（`listSinceLastSuccess`）。最後の成功だけを見ていると、**途中で失敗した適用が書いたファイルを「skillamのものではない」と誤判定し、再試行が永久に409で止まる**

### Phase 3b 以降に送った項目

- **preview と apply の乖離検出**: `/apply` はプランを作り直して実行するため、プレビュー後にプロジェクトやロールが変わると、ユーザーが確認した内容と異なる計画が適用されうる。プレビュー時に入力のハッシュを返し、適用時に照合して 409 を返す案。危険な上書きは上記の衝突検出が既に止めるため、UIができる Phase 4 に合わせて実装する
- **プロジェクト単位の適用ロック**: 現状は better-sqlite3 と `node:fs` の同期API、Keychain の `execFileSync` によりハンドラ内で `await` が発生せず、結果として同時適用が直列化されている。この不変条件はコード変更で容易に壊れるため、明示的なミューテックスを将来入れる
- `enabledPlugins` の管理（ロールにプラグイン概念が必要）
