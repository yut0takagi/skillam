...
段階1から持ち越していた2つを画面に出し、**UI から3経路すべてが使える
ようになった**。

- **合成適用を既定に** — 適用画面は `roleId` を送らなくなった。
  スコープ / グループ / 直接のすべてが合成される
- **`ApplyPlan.suppressedAllow`** — `composeRoles` は返していたが plan に
  載っていなかった。載せて、プレビューに「deny で落ちた許可」として出す
- **出どころ表示** — 各 Skill / Agent / MCP がどの経路から来たかを出す
- **グループ / スコープの管理画面** — 左ナビに独立項目として追加

### 見つけた問題（段階4で判明）

**UI が段階1〜3をまるごと使えていなかった。** 適用画面は常に明示的な
`roleId` を送っており、明示 `roleId` は全バインディング経路を迂回する。
つまりグループもスコープも複数ロール合成も、API にはあるのに画面からは
一切届かなかった。「適用は1ロールずつです。複数ロールの合成は未対応」
という案内文も残っていた（段階1で事実でなくなっていた）。

サーバだけ作って UI を後回しにすると、**機能が存在しないのと同じ状態が
段階3つ分積み上がる**。段階4を「表示を足すだけ」と見積もっていたが、
実際には経路の接続そのものが未了だった。

## 次にやること

段階1〜4で設計書の範囲は終わっている。段階4の持ち越しだった2件も
PR #28 で解消した。

- **グループ所属の編集**（#20）— プロジェクト詳細のサイドバーから所属を
  組めるようになった。`setProjectGroups` は段階2からあったが未接続だった
- **スコープの対象PJT表示**（#21）— `GET /scopes/:id/projects` を追加し、
  スコープ画面に「対象PJT」を置いた。判定はパスだけで行い `scope_roles` を
  経由しない（ロール未設定のスコープでも当たる先は答えられるべきなので）。
  除外済みは印つきで表示する

**ドリフト検知の MCP 定義比較**（#22）も対応した。`managed_json` に
`mcpDefinitions` を足し、`command` / `args` の書き換えを
`mcp-server-changed` として報告する。`env` はキー名だけを記録する
（理由は罠6）。古い履歴行は `mcpDefinitions` が無くても読めるので、
従来どおり消失だけは検出できる。

残りは設計書の「決めていないこと」と、GitHub の Issue にある。

- グループの入れ子、スコープの除外は設計書のとおり保留
- 配布まわり（#23 自動更新 / #25 universal / #27 Homebrew Cask）
- Windows / Linux が未検証（#24）
...
読まれず、`document is not defined` で大量に落ちていた。

**解決済み**。ルートの `vitest.config.mts` が両 workspace を projects として
束ねるので、どちらから叩いても各 workspace の設定が使われる。

注意書きで回避し続けるのではなく設定で潰した。失敗の出方（大量の赤）が
「テストが壊れている」と誤読させる罠は、人が覚える前提にしないほうがよい。
...
### 6. 「ロールの env は必ず `secret_ref:` になっている」は嘘だった

#22 で MCP 定義を `managed_json` に記録するとき、env をそのまま入れた。
`extractSecretsFromEnv` が実値を `secret_ref:` に置き換えるので記録に
平文は入らない、と考えたため。

**この前提が誤り。** `extractSecretsFromEnv` を呼ぶのは
`catalog.routes.ts`（カタログ取り込み）**だけ**で、
`PUT /roles/:id/mcp-servers` で直接足したサーバーは生の値を保持する。
結果、適用履歴に平文のトークンを書き込む実装になっていた。

自分で書いた「履歴に平文シークレットが入らないこと」のテストが落ちて発覚。
**思い込みをテストにしておいたから捕まった。** 通ると思って書いたテストが
落ちるのは、実装ではなく前提が間違っているという合図。

対応は env の**値を記録しないこと**（`envKeys` としてキー名だけ持つ）。
その代償として env の値の書き換えは検出できない。README に明記した。
「安全側に倒すと検知能力が落ちる」というトレードオフを、黙って選ばずに
書き残すこと。

### 7. 記録と書き込み対象がオブジェクトを共有していた

同じ #22 で、`planMcp` が `.mcp.json` に書く定義と記録用の定義に
**同一の参照**を入れていた。`resolveSecretRefs` は今はコピーを作るので
実害は出ていなかったが、呼び出し側が将来その場で書き換えれば、記録側の
`secret_ref:` が復号後の値に化ける。

「今の呼び出し側は安全だから大丈夫」で通そうとして、念のため書いた
回帰テストで落ちた。`structuredClone` で切って解決。
**記録の正しさを、呼び出し側の作法に依存させない。**
...
現在: server 631 / web 146 通過（計 777）、型は server・web とも clean。
