...
段階1〜4で設計書の範囲は終わっている。残りは設計書の「決めていないこと」
と、下記の持ち越し。

- **プロジェクトのグループ所属を UI から編集できない。** 一覧表示だけで、
  変更は `PUT /projects/:id/groups` を直接叩く必要がある
  （API は段階2で作成済み）
- **スコープがどのプロジェクトに当たっているかが見えない。** スコープ画面は
  パスとロールだけを出す。当たっている PJT 一覧があると確認しやすい
  （グループ側には `GET /groups/:id/projects` があるが、こちらも未使用）
