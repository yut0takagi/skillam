...
MCP サーバーは**名前が消えたか**だけでなく、**定義が書き換えられたか**も見る。`command` / `args` が差し替えられていれば `mcp-server-changed` として報告する。MCP サーバーは外部プロセスを起動するので、`command` を差し替えられて緑を返すのはドリフト検知の役割を果たせていない。

`env` は**キー名だけを記録し、値は記録しない**。ロールの env の値は必ずしも `secret_ref:` ではなく（カタログ取り込み以外の経路では生の値が入る）、記録は適用履歴に残るため。したがって env のキーの増減は検出するが、**値の書き換えは検出しない**。値まで見るにはマスターキーでの復号が必要で、読み取り専用の点検にそれを持ち込まない判断。
